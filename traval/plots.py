...
class ComparisonPlots:
    ...
    def plot_series_comparison(
        ...
        # Plot both series
        for s, c, ls in zip(
            [self.cp.s1n, self.cp.s2n], ["gray", "k"], ["solid", "dashed"]
        ):
            ...
            if mark_different:
                s_diff = s.copy()
                not_diff = s_diff.index.difference(self.cp.idx_in_both_different)
        ...
        if mark_identical:
            ...
            not_identical = s_identical.index.difference(self.cp.idx_in_both_identical)
            ...
            if self.cp.idx_in_s1.size > 0:
                (p3,) = ax.plot(
                    ...
                    **self.color_dict["only_in_s1"],
                    label="only in series 1: {}".format(self.cp.s1n.name),
                )
            ...
            if self.cp.idx_in_s2.size > 0:
                (p4,) = ax.plot(
                    ...
                    **self.color_dict["only_in_s2"],
                    label="only in series 2: {}".format(self.cp.s2.name),
                )
    ...
    def plot_relative_comparison(
        ...
        if self.cp.idx_r_flagged_in_both.size > 0:
            ...
            s_base.loc[self.cp.idx_r_flagged_in_both] = self.cp.basen.loc[
                self.cp.idx_r_flagged_in_both
            ]
            (p6,) = ax.plot(
                s_base.index,
                s_base,
        ...
        if mark_introduced:
            intro_idx = self.cp.idx_r_introduced_in_s2.union(
                self.cp.idx_r_introduced_in_both
            )
            if (self.cp.idx_r_introduced_in_s1.size > 0) or (intro_idx.size > 0):
                ax.plot(
                    self.cp.s1n.loc[self.cp.idx_r_introduced_in_s1].index,
                ...
                (p7,) = ax.plot(
                    self.cp.s1n.loc[intro_idx].index,
        ...
        ax.legend(plot_handles, plot_labels, loc=(0, 1), ncol=max(len(plot_handles), 5))
...
def roc_plot(
    ...
    **kwargs
    ...
    for itpr, ifpr, ilbl in zip(tpr, fpr, labels):
        sc = ax.scatter(ifpr, itpr, s=6**2, c=colors, marker="o", label=ilbl, **kwargs)
...
def det_plot(fpr, fnr, labels, ax=None, **kwargs):
    ...
    for ifpr, ifnr, ilbl in zip(fpr, fnr, labels):
        ax.plot(
            norm.ppf(ifpr), norm.ppf(ifnr), marker="o", ls="none", label=ilbl, **kwargs
        )
